...
export default function NuevaTarea() {
  ...
  useEffect(() => {
    ...
    const cargarDatos = async () => {
      try {
        const tareaRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/tareas/${tareaIdUrl}`);
        const tarea = await tareaRes.json();

        const stRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/servicios_tarea/por-tarea/${tareaIdUrl}`);
        ...
        setValores((prev) => ({
          ...
          cantidadMateriales: servicioTarea.cantidadMateriales?.toString() || '',
  ...
  useEffect(() => {
    ...
    const cargarMateriales = async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/st_material/por-servicio/${servicioTareaId}`);
  ...
  const handleSubmit = async () => {
    try {
      ...
      const resUpdateST = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/servicios_tarea/${servicioTareaId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(bodyUpdateST),
      });

      if (!resUpdateST.ok) throw new Error('Error actualizando servicios_tarea');
  ...
  return (
    <div className="contenedor-formulario">
      ...
      <TablaListado
        ...
        onEditar={(m) => router.push(`/materiales/STMaterial/${m.id}?edit=true`)}
