...
const TreeViewServiciosTareas: React.FC<Props> = ({ obraId }) => {
  ...
  const totalGeneral = servicios.reduce(
    (acc, serv) =>
      acc +
      (serv.serviciosTarea ?? []).reduce((sum, st) => sum + (st.total || 0), 0),
    0
  );
  ...
  const handleEliminarServicio = async (servicioId: number) => {
    const confirmar = window.confirm(
      "¿Eliminar este servicio y todas sus tareas?"
    );
    if (!confirmar) return;
  ...
  const handleEliminarTarea = async (tareaId: number) => {
    ...
    try {
      ...
      setServicios((prevServicios) =>
        prevServicios.map((s) => ({
          ...s,
          serviciosTarea: s.serviciosTarea.filter(
            (st) => st.tarea?.id !== tareaId
          ),
        }))
      );
    } catch (err) {
  ...
  return (
    <div className={styles.contenedor}>
      <div className={styles.header}>
        ...
        <Link
          href={`/obras/servicios/createSOT?obraId=${obraId}`}
          ...
          return (
            <li key={serv.id}>
              <div className={styles.nodo}>
                ...
                <span className={styles.totalServicio}>
                  Total: {totalServicio.toFixed(2)} €{" "}
                </span>
                <Link
                ...
                <Link
                  href={`/obras/tareas/create?servicioId=${serv.id}&obraId=${obraId}`}
                  className={styles.btnImprevisto}
                >
                  + Imprevisto
                </Link>
                <Link
                  href={`/obras/tareas/create?servicioId=${serv.id}&obraId=${obraId}`}
                  className={styles.btnGasto}
                >
                  + Gasto
                </Link>
                <button
                  ...
                  {(serv.serviciosTarea ?? []).map((st) => (
                    <li key={st.id} className={styles.tarea}>
                      ...
                      <span className={styles.fechas}>
                        ({st.fechaInicio?.substring(0, 10)} →{" "}
                        {st.fechaFin?.substring(0, 10)})
                      </span>
